    ...
    <div>
        ...
        <div class="row">
            ...
            <table class="table table-striped table-bordered" style="width:100%">
                ...
                <?php
                $k=1;
                function getmov($mov_id)
                {
                    global $conn;
                    $sql=mysqli_query($conn,"select mov from output_indicators where id='$mov_id'");

                    return mysqli_fetch_assoc($sql)['mov'];

                }

                function getindicator( $mov_id)
                {
                    global $conn;
                    $sql=mysqli_query($conn,"select output_id from output_indicators where id='$mov_id'");

                    return mysqli_fetch_assoc($sql)['output_id'];
                }

                while ($row=mysqli_fetch_assoc($resultmovcomment)){ ?>
                    <tr>
                        <td><?= $k?></td>
                        <td><?= getpreparedby($row['added_by'])?></td>
                        <td><?= prntmov(json_decode(getmov($row['mov_id'])),getindicator($row['mov_id'])) ?></td>
                        <td><?= $row['mov_comment'] ?></td>
